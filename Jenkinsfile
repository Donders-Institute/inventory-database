    ...
    stages {
        ...
        stage('Staging') {
            ...
            steps {
                ...
                withCredentials([
                    usernamePassword (
                        credentialsId: params.PROJECT_DATABASE_CREDENTIALS,
        ...
        stage('Tag and push (PRODUCTION)') {
            ...
            steps {
                ...
                withCredentials([
                    ...
                    script {
                        def result = sh(script: "git ls-remote https://${GITHUB_USERNAME}:${GITHUB_PASSWORD}@github.com/Donders-Institute/inventory-database.git refs/tags/${params.PRODUCTION_GITHUB_TAG}", returnStdout: true).trim()
                        if (result != "") {
                            sh "git push --delete https://${GITHUB_USERNAME}:${GITHUB_PASSWORD}@github.com/Donders-Institute/inventory-database.git ${params.PRODUCTION_GITHUB_TAG}"
                            echo "Removed existing remote tag ${params.PRODUCTION_GITHUB_TAG}"
                        }
                    }

                    // Create remote tag
                    sh "git push https://${GITHUB_USERNAME}:${GITHUB_PASSWORD}@github.com/Donders-Institute/inventory-database.git ${params.PRODUCTION_GITHUB_TAG}"
